...
async function load() {
  ...
  try {
    const [routeResult, channels, providers, models] = await Promise.all([
      api<{ data: ModelRoute[] }>("/routes"), api<{ data: Channel[] }>("/channels"), api<{ data: Provider[] }>("/providers"),
...
function create() {
  editing.value = null;
  Object.assign(form, { publicModel: "", providerId: "", upstreamModel: "", endpoint: "chat", enabled: true, priority: 100, weight: 1 });
  modal.value = true;
}
function edit(row: ModelRoute) {
  editing.value = row;
  Object.assign(form, {
    publicModel: row.public_model, providerId: row.provider_id, upstreamModel: row.upstream_model,
    endpoint: row.endpoint, enabled: row.enabled === 1, priority: row.priority, weight: row.weight,
  });
  modal.value = true;
}
function managed(row: ModelRoute): boolean {
  try { return JSON.parse(row.options_json || "{}").managed_by === "provider-model-selection"; }
  catch { return false; }
}
async function save() {
  try {
...
async function remove(id: string) {
  try { await api(`/routes/${id}`, { method: "DELETE" }); message.success("路由已删除"); await load(); }
  catch (error) { message.error(error instanceof Error ? error.message : String(error)); }
}
async function recover(providerId: string) {
  try { await api(`/routes/provider/${providerId}/recover`, { method: "POST" }); message.success("已清除该供应商的熔断状态"); await load(); }
  catch (error) { message.error(error instanceof Error ? error.message : String(error)); }
}
...
const columns: DataTableColumns<ModelRoute> = [
  { title: "客户端模型名", key: "public_model", render: (row) => h("div", [
    h("strong", row.public_model),
    managed(row) ? h(NTag, { size: "tiny", style: "margin-left:8px" }, { default: () => "供应商自动管理" }) : null,
  ]) },
  { title: "实际上游", key: "target", render: (row) => h("div", [
    h(NTag, { size: "small" }, { default: () => row.provider_id }),
    h("span", { class: "mono", style: "margin-left:8px" }, row.upstream_model),
  ]) },
  { title: "分流规则", key: "priority", render: (row) => h("div", [
    h("strong", `优先级 ${row.priority}`), h("div", { class: "muted", style: "font-size:12px" }, `同级权重 ${row.weight}`),
  ]) },
  { title: "可用状态", key: "availability", render: (row) => {
    const state = row.availability?.status ?? "ready";
    const type = state === "ready" ? "success" : state === "degraded" ? "warning" : "error";
    const label = state === "ready" ? "可用" : state === "degraded" ? "部分可用" : "已摘除";
    const detail = row.availability?.reason || `${row.availability?.availableCredentials ?? 0}/${row.availability?.totalCredentials ?? 0} 个账号可用`;
    return h("div", [h(NTag, { size: "small", type }, { default: () => label }), h("div", { class: "muted", style: "font-size:12px;margin-top:4px;max-width:280px" }, detail), row.availability?.retryAt ? h("div", { class: "muted", style: "font-size:12px" }, `预计恢复：${formatRetry(row.availability.retryAt)}`) : null]);
  } },
  ...
  { title: "操作", key: "actions", render: (row) => h(NSpace, null, { default: () => [
    ...
    row.health?.disabledUntil && row.health.disabledUntil > Date.now()
      ? h(NButton, { size: "small", type: "warning", secondary: true, onClick: () => recover(row.provider_id) }, { default: () => "立即恢复" })
      : null,
    !managed(row) ? h(NPopconfirm, { onPositiveClick: () => remove(row.id) }, {
...
<template>
  <page-header title="模型路由" description="把客户端模型名指向一个或多个上游；系统会自动避开额度耗尽、冷却和熔断节点。">
    <n-button type="primary" @click="create"><template #icon><plus /></template>新增手动路由</n-button>
    <n-button :loading="loading" @click="load"><template #icon><refresh-cw /></template>刷新</n-button>
  </page-header>
  <n-alert type="info" :bordered="false" style="margin-bottom:16px">
    <strong>怎么用：</strong>相同“客户端模型名”的线路会组成一个路由池。数字更小的优先级先用；只有主线路不可用时才切到更大的优先级。
    同一优先级按权重分流。OpenAI 供应商的模型勾选与别名建议直接在“OpenAI-compatible 供应商”里配置，这里只做高级主备覆盖。
  </n-alert>
  <n-card><n-data-table :columns="columns" :data="rows" :loading="loading" :scroll-x="1180" /></n-card>

  <n-modal v-model:show="modal" preset="card" :title="editing ? '编辑手动路由' : '新增手动路由'" style="width:min(720px,calc(100vw - 32px))">
    <n-form label-placement="top">
      <n-form-item label="客户端看到的模型名"><n-input v-model:value="form.publicModel" placeholder="例如 coding-fast" /></n-form-item>
      <div class="grid-2">
        <n-form-item label="供应商 / 渠道"><n-select v-model:value="form.providerId" :options="sources" filterable /></n-form-item>
        <n-form-item label="实际上游模型"><n-select :value="form.upstreamModel" :options="upstreamOptions" filterable tag placeholder="选择已发现模型，或直接输入模型 ID" @update:value="selectUpstream" /></n-form-item>
      </div>
      <div class="grid-stats" style="grid-template-columns:repeat(3,1fr)">
        <n-form-item label="端点"><n-select v-model:value="form.endpoint" :options="endpoints" /></n-form-item>
