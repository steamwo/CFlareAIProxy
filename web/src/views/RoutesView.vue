...
<template>
  <page-header title="模型路由" description="把客户端模型名指向一个或多个上游；系统会自动避开额度耗尽、冷却和熔断节点。"><n-button type="primary" @click="create"><template #icon><plus /></template>新增手动路由</n-button><n-button :loading="loading" @click="load"><template #icon><refresh-cw /></template>刷新</n-button></page-header>
  ...
  <n-card><n-data-table :columns="columns" :data="rows" :loading="loading" :pagination="tablePagination" :scroll-x="1180" /></n-card>
