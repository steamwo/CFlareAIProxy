...
const summaryPagination = { pageSize: 5, pageSizes: [5, 10, 20], showSizePicker: true };
const fmt = (value: number) => Intl.NumberFormat("zh-CN", { notation: value > 9999 ? "compact" : "standard", maximumFractionDigits: 1 }).format(value || 0);
const money = (micros: number) => `$${(micros / 1_000_000).toFixed(4)}`;
const providerColumns: DataTableColumns<any> = [{ title: "供应来源", key: "provider_id" }, { title: "请求", key: "requests", render: (row) => fmt(row.requests) }, { title: "Token", key: "tokens", render: (row) => formatTokens(row.tokens) }];
const modelColumns: DataTableColumns<any> = [{ title: "模型", key: "public_model", ellipsis: { tooltip: true } }, { title: "请求", key: "requests", render: (row) => fmt(row.requests) }, { title: "Token", key: "tokens", render: (row) => formatTokens(row.tokens) }];
async function load() { loading.value = true; try { data.value = await api<Overview>("/overview"); } catch (error) { message.error(error instanceof Error ? error.message : String(error)); } finally { loading.value = false; } }
...
<template>
  <page-header title="运行概览" description="过去 24 小时的网关状态、调用量与账号资源。"><n-button :loading="loading" @click="load"><template #icon><refresh-cw /></template>刷新</n-button></page-header>
  <div v-if="loading && !data" class="grid-stats"><n-card v-for="index in 4" :key="index"><n-skeleton text :repeat="2" /></n-card></div>
  <template v-else-if="data">
    <div class="grid-stats"><n-card><div class="metric">{{ fmt(data.usage24h.requests) }}</div><div class="metric-label">24 小时请求</div></n-card><n-card><div class="metric">{{ data.usage24h.successRate.toFixed(1) }}%</div><div class="metric-label">成功率</div><n-progress type="line" :percentage="data.usage24h.successRate" :status="successType" :show-indicator="false" style="margin-top:10px" /></n-card><n-card><div class="metric">{{ formatTokens(data.usage24h.tokens) }}</div><div class="metric-label">Token 使用量</div></n-card><n-card><div class="metric">{{ money(data.usage24h.costMicros) }}</div><div class="metric-label">估算成本</div></n-card></div>
    ...
    <div class="grid-2"><n-card title="来源调用"><n-data-table v-if="data.providerUsage.length" :columns="providerColumns" :data="data.providerUsage" :pagination="summaryPagination" :bordered="false" /><n-empty v-else description="暂无调用记录" /></n-card><n-card title="热门模型"><n-data-table v-if="data.modelUsage.length" :columns="modelColumns" :data="data.modelUsage" :pagination="summaryPagination" :bordered="false" /><n-empty v-else description="暂无调用记录" /></n-card></div>
  </template>
